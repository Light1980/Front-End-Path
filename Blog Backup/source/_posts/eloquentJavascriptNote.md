...
根据这两位的谜之转换方法，我们就能使用一个被称作**短路评估(short-circuit evaluation)**的技术（看到第三章发现这个真有用）。
...
比之用`loop`循环写的函数，这个新函数在数学上明显要显得优雅许多（好吧我确实觉得差不多，但直觉上要舒服些）。新的`power`函数将根据传入的参数来决定它迭代的次数。
